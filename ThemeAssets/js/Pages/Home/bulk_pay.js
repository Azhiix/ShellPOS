...
var bulk_pay = function () {
    return {
        ...
        init: function () {
            $(document).ready(function () {
                ...
                $('#downloadExcel').click(function () {
                    bulk_pay.downloadTableAsExcel('bulkTable', 'BulkPayments.xlsx');
                });
                ...
                $('.show-modal').on('click', function (e) {
                    ...
                    bulk_pay.finalisepayment(); // Now this will handle deciding whether to show the modal or not.
                });
            });
        },
        ...
        initializePaymentTypeDropdown: function () {
            $('.btn').on('click', function () {
                ...
                if (selectedType !== 'all') {
                    table.column(6).search(selectedType, true, false).draw();
                } else {
        ...
        downloadTableAsExcel(tableId, filename = 'table.xlsx') {

            // Clone the original table
            var originalTable = document.getElementById(tableId);
            var clonedTable = originalTable.cloneNode(true);

            // Remove the column to ignore from each row of the cloned table
            Array.from(clonedTable.rows).forEach(row => {
                if (row.cells.length > 1) { // Check if the column exists
                    row.deleteCell(1); // Delete the cell at the specified index
                }
            });

            // Convert the cloned table with the column removed to a workbook
        ...
        preloadModal: function () {
            ...
            // Removed showBtn event listener setup to prevent automatic modal showing
        ...
        generateBulkFile: function () {
            ...
            if (pmntDate === "" || !pmntDate) {
                ...
                //first line
                var paymentTxt = '0|BP-V2|' + date + '|' + purposeCode +'|1|' + bulkPmntAmt.toFixed(2) + '|' + BulkCounter + '|' + bulkPmntAmt.toFixed(2) + '|MUR|' + date + '095122\r\n';
                //second line
