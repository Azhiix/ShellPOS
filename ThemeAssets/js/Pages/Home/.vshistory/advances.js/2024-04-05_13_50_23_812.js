...
function checkIfAdvanceExists() {
    return new Promise(function (resolve, reject) {
        ...
        if (!empPin || !($("#firstMonthDeductID").val()) || !$("#idLastMonthDeduct").val() || !AmountMonthsDeduct || !amntTaken) {
            displayWarningMessage("Please fill in all the required fields.");
...
function initiateSaveAdvanceContract() {
    checkIfAdvanceExists().then(function (AdvanceNotExist) {
        if (AdvanceNotExist) {
            ...
            Swal.fire({
                title: "Advance already exists!",
                text: "An advance for this user for this period already exists. Do you want to continue adding?",
                icon: "warning",
                showCancelButton: true,
