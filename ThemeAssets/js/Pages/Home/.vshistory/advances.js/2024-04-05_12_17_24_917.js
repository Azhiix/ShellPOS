...
function checkIfAdvanceExists() {
    return new Promise((resolve, reject) => {
        ...
        console.log("Checking advances for employee PIN:", empPin); // Log #1

        jQuery.ajax({
            type: "GET",
            url: "advances.aspx/checkIfAdvanceExists",
            data: JSON.stringify({ emp_pin: empPin }),
            contentType: "application/json; charset=utf-8",
            dataType: "JSON",
            success: function (msg) {
                console.log("Received response:", msg); // Log #2
                if (msg.d == 1) {
                    Swal.fire({
                        title: 'Existing Advance',
                        text: 'You Already Have An advance for this employee. Proceed?',
                        icon: 'warning',
                        showCancelButton: true,
                        ...
                        console.log("Swal result:", result); // Log #3
                        if (result.value) {
                            resolve(true); // User decides to proceed
                        } else {
                            resolve(false); // User decides not to proceed
                        }
                    });
                } else {
                    resolve(true); // No existing advance, proceed to save
                }
            },
            failure: function (msg) {
                console.log("AJAX call failed with message:", msg); // Log #4
                Swal.fire('Error!', 'An error has occurred.', 'error');
                reject(new Error("An error occurred while checking for existing advances."));
            }
        });
    });
}
...
checkIfAdvanceExists().then((proceed) => {
        if (proceed) {
            saveAdvanceContract();
        }
    }).catch((error) => {
...
function saveAdvanceContract() {
    // Gather all the necessary data from your form or UI elements
    var empPin = $("#employeeList").val();
    ...
    // Prepare your data for the AJAX call
    ...
    // Make the AJAX call to your server-side method
    jQuery.ajax({
        type: "POST",
        ...
        success: function (msg) {
            if (msg.d > 1) {
                Swal.fire('Success!', 'Advance contract successfully saved.', 'success').then(function () {
                    loadAdvances(); // Refresh table data after saving
                });
            } else if (msg.d > 0) {
                Swal.fire('Notice!', 'Advance saved, but there was a notice you might want to address.', 'info').then(function () {
                    loadAdvances(); // Refresh table data after saving
                });
            } else if (msg.d == 9) {
                Swal.fire('Error!', 'An error has occurred during saving.', 'error');
            }
        },
        failure: function (msg) {
            Swal.fire('Error!', 'An error has occurred and the save operation failed.', 'error');
        }
    });
}
