    ...
    $.ajax({
        ...
        success: function (successmsg) {
            ...
            $.each(successmsg.d, function (i, item) {
                var status = await checkIfAdvanceExists(item.empPin)
                var statusBadge = status ? "<span class='badge badge-warning'>Existing Advance</span>" : findAdvanceStatus(item);
...
function saveAdvanceContract() {
    checkIfAdvanceExists().then(canProceed => {
        if (!canProceed) {
            // User decided not to proceed or check failed
            console.log("Not proceeding with saving the advance.");
            ...
            success: function (msg) {
                if (msg.d > 1) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            Swal.fire('Saved!', 'Advance contract successfully saved.', 'success')
                                .then(function () {
                                    loadAdvances();
                                });
                        }
