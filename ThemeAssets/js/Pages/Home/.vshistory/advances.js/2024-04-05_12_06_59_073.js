...
function saveAdvanceContract() {
    ...
    jQuery.ajax({
        type: "POST",
        url: "advances.aspx/saveAdvanceContract", //It calls our web method  
        data: JSON.stringify({
            emp_pin: empPin, dateChosen: dateChosen, amntTaken: amntTaken, firstMonthDeduct: firstMonthDeduct, finalMonthDeduct: finalMonthDeduct, installmentsPerMonth: installmentsPerMonth, amtMonthsTaken: AmountMonthsDeduct
        }),
        contentType: "application/json; charset=utf-8",
        dataType: "JSON",
        success: function (msg) {
            if (msg.d > ) {
                displayWarningMessage("Warning, You already have an advance ");
                loadAdvances();
            }
            if (msg.d == 0) {
                displayWarningMessage("Advance contract not saved: Please delete the entry that is conflicting with the new one");
            }
            if (msg.d == 9) {
