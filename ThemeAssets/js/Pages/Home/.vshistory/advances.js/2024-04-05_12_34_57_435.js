...
function initiateSaveAdvanceContract() {
    checkIfAdvanceExists().then(function (shouldProceed) {
        if (shouldProceed) {
            saveAdvanceContract();
        } else {
            console.log("Advance creation cancelled by user or due to existing advance.");
            // Optionally, show a message to the user that the process is cancelled or why it's cancelled
        }
