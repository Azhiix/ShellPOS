    ...
    return {
        ...
        downloadTableAsExcel(tableId, filename = 'table.xlsx') {
            ...
            Array.from(clonedTable.rows).forEach(row => {
                if (row.cells.length > ignoreColumnIndex) { // Check if the column exists
                    row.deleteCell(ignoreColumnIndex); // Delete the cell at the specified index
                }
            });
            ...
            // Cleanup: Remove the cloned table if you added it to the DOM
            // If you didn't add it to the DOM, this step can be omitted.
