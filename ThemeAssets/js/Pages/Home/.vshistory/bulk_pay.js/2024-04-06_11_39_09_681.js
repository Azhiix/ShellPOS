    ...
    return {
        ...
        initializePaymentTypeDropdown: function () {
            $('.btn').on('click', function () {
                ...
                console.log("Selected Type:", selectedType); // Debugging: Check the selected value

                // Normalize case for comparison to avoid case sensitivity issues
                selectedType = selectedType.toLowerCase();

                if (selectedType !== 'all') { // Assuming 'All' option value is "All", now compared in lowercase
                    // Apply search. Ensure you adjust the column index correctly.
                    // Debugging: Log to see if it enters the condition
                    console.log("Filtering for:", selectedType);
                    table.column(3).search(selectedType, true, false).draw();
                } else {
                    // Debugging: Log to see if it enters the 'else' part
                    console.log("Resetting filter");
                    table.search('').columns().search('').draw(); // Clear any filters
                }
            });
        },
