    ...
    return {
        ...
        initializePaymentTypeDropdown: function () {
            ...
            $('#enterButton').on('click', function () {
                ...
                var table = $('#bulkTable').DataTable(); // Make sure this targets your DataTable correctly

                // Apply the filter based on the selected type
                if (selectedType !== 'All') {
                    // Assuming the payment type information is in the third column (index 2)
                    // Adjust the index as necessary to match your table's structure
                    table.column(2).search(selectedType).draw();
                } else {
                    // If 'All' is selected or to clear the filter
                    table.draw();
                }
            });
        },
