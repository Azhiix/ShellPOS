...
const handleRefresh = () => {
    ...
    jQuery.ajax({
        ...
        data: JSON.stringify({ emp_pin: employeeId }), // Corrected data parameter
        contentType: "application/json; charset=utf-8",
        dataType: "JSON",
        success: function (msg) {
            if (msg.d != null) {
                var emp = msg.d;
                var imageData = `data:image/png;base64,${emp.img}`; // Assuming image is PNG. Adjust MIME type if necessary.
                $(".empFNameInput").val(emp.emp_firstname);
                $("#add-emp-image").attr('src', imageData); // Correctly formatted image data
            } else {
