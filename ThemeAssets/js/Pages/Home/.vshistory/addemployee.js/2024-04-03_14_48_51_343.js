...
// Function to handle the refresh icon click event
let canRefresh = true; // Variable to track if refresh is allowed

// Ensure to have this line executed after the document is fully loaded
$(document).ready(function () {
    // Delegate event for dynamically added refresh icon
...
const handleRefresh = () => {
    if (!canRefresh) return; // Exit if refresh is not allowed

    var employeeId = $(".empPinNoInput").val();

    jQuery.ajax({
        type: "GET",
        url: "addemployee.aspx/loadEmployeeImage",
        data: JSON.stringify({ emp_pin: employeeId }), // Ensure data is properly stringified
        contentType: "application/json; charset=utf-8",
        dataType: "JSON",
        success: function (msg) {
            if (msg.d != null) {
                var emp = msg.d;
                var newImage = `data:image/png;base64,${emp.img}`;
                $(".empFNameInput").val(emp.emp_firstname);

                // Use jQuery to update HTML content and ensure event handlers are managed properly
                $(".addempImage").html(`<img src="${newImage}" class='add-emp-image' alt="Embedded Image" style="border-radius: 80px;">
                <i class="fas fa-sync-alt refresh-icon" ${canRefresh ? '' : 'style="display: none;"'}></i>`); // Conditionally show/hide the icon based on canRefresh

            } else {
                displayErrorMessage("Failed to retrieve employee details.");
            }
        },
        failure: function (msg) {
            displayErrorMessage("Error during request: " + msg, true);
        }
    });

    canRefresh = false; // Disable refresh
    setTimeout(() => {
        canRefresh = true; // Re-enable refresh after 2 minutes
        $('.refresh-icon').show(); // Ensure the refresh icon is shown again after timeout
...
<i class="fas fa-sync-alt refresh-icon" onclick="handleRefresh()"></i>
    ...
    $(".empPinNoInput").prop('disabled', true);
