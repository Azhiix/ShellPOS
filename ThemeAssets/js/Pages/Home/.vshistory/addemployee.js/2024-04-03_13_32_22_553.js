...
const handleRefresh = () => {
    ...
    jQuery.ajax({
        type: "POST", // Change to POST if your server-side method expects it
        url: "addemployee.aspx/loadEmployeeImage",
        data: JSON.stringify({ emp_pin: employeeId }), // Correctly format data as JSON
        contentType: "application/json; charset=utf-8",
        dataType: "JSON",
        ...
        success: function (response) {
            if (response.d != null) {
                ...
                // Assuming the response includes the image in a property named 'img'
                // and it's already base64 encoded
                var imageSrc = "data:image/png;base64," + emp.img; // Update to match the actual property name if different
                $("#empFaceImg").attr('src', imageSrc); // Update the src attribute of the image
