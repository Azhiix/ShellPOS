document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('generatePDF').addEventListener('click', function () {
        ...
        doc.text("BRN:", 10, 50);
        doc.text("Client ID: " + clientId, 10, 55);

        const dateFrom = document.getElementById('dateFrom').value;
        const dateTo = document.getElementById('dateTo').value;

        doc.text("Invoice Date From: " + dateFrom, 150, 40);
        doc.text("Invoice Date To: " + dateTo, 150, 45);
        ...
        gridApi.forEachNode(function (node) {
            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            /*doc.text(node.data.ItemCode, 10, y);*/
        ...
        // Add a summary section, assuming data is available
        doc.setFont('helvetica', 'bold');
        doc.text("Total (Excl)", 150, y);
        // Suppose you have a function or a calculation for total
        doc.setFont('helvetica', 'normal');
        doc.text("Calculate Total Excl here", 180, y);

        // Add hours worked if relevant
        doc.setFont('helvetica', 'bold');
        doc.text("Hours Worked:", 10, y + 20);
        doc.setFont('helvetica', 'normal');
        doc.text("Input hours here", 50, y + 20);

        // Bank details and footer, as you have set up above
