...
function triggerCustomSwal(title, text, icon, selector) {
    const fieldOffset = $(selector).offset();
    ...
    customSwal.fire({
        ...
        onOpen: () => {
            ...
            swalElement.style.top = `${fieldOffset.top - windowScrollTop + $(selector).outerHeight()}px`;
            swalElement.style.left = `${fieldOffset.left}px`;
        }
    });
}
...
function validateSale(saleId) {
    ...
    if (!productType) {
        triggerCustomSwal('Error!', 'Please select a product type.', 'error', `#sale${saleId} .clsProductType`);
        return false;
    }

    if (!item) {
        triggerCustomSwal('Error!', 'Please select an item.', 'error', `#subCategory${saleId}`);
        return false;
    }

    if (!quantity) {
        triggerCustomSwal('Error!', 'Please enter a valid quantity.', 'error', `#quantityAmount${saleId}`);
        return false;
    }
...
$(document).ready(function () {
    ...
    initializeVehicleRegSelect();
    initializeDriverNameSelect();
...
function initializeVehicleRegSelect() {
    ...
    $('#vehicleRegSelect').on('select2:select', function (e) {
        if (e.params.data.id === 'Other') {
            appendCustomInputField('#vehicleRegSelect', 'vehicle');
        }
    });
}

function initializeDriverNameSelect() {
    ...
    $('#driversNameSelect').on('select2:select', function (e) {
        if (e.params.data.id === 'Other') {
            appendCustomInputField('#driversNameSelect', 'driver');
        }
    });
}

function appendCustomInputField(selector, type) {
    // Remove any existing custom input field for the specific type
    $(`.custom-input-wrapper-${type}`).remove();

    // Append a custom input field below the select2 dropdown
    $(selector).after(`
        <div class="custom-input-wrapper-${type} mt-2">
            <input type="text" id="customInput${type}" class="form-control" placeholder="Enter custom value" />
            <button type="button" class="btn btn-primary mt-2" id="addCustomValueButton${type}">Add</button>
        </div>
    `);

    // Handle the custom value submission
    $(`#addCustomValueButton${type}`).on('click', function () {
        const customValue = $(`#customInput${type}`).val().trim();
        if (customValue) {
            const newOption = new Option(customValue, customValue, true, true);
            $(selector).append(newOption).trigger('change');
            // Remove the custom input field after adding the value
            $(`.custom-input-wrapper-${type}`).remove();
        }
    });
}
...
function appendCustomInputField(selector) {
    // Remove any existing custom input field
    $('.custom-input-wrapper').remove();

    // Append a custom input field below the select2 dropdown
    $(selector).after(`
        <div class="custom-input-wrapper mt-2">
            <input type="text" id="customInput" class="form-control" placeholder="Enter custom value" />
            <button type="button" class="btn btn-primary mt-2" id="addCustomValueButton">Add</button>
        </div>
    `);

    // Handle the custom value submission
    $('#addCustomValueButton').on('click', function () {
...
function promptCustomValue(label, selector) {
    Swal.fire({
        ...
        inputValidator: (value) => {
            ...
            $(selector).append(new Option(value, value, true, true)).trigger('change');
        }
    });
}
...
function populateClientAndVehicleRegDropdown() {
    ...
    clientSelect.on('change', function () {
        ...
        vehicleRegSelect.append(new Option('Other', 'Other'))
        ...
        driversNameSelect.append(new Option('Other', 'Other'))
