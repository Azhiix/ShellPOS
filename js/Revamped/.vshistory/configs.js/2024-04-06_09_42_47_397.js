...
function LoadPersonalDetails(msg) {
    ...
    $(".clsBankNum").on('submit', function () {
        var currentValue = $(this).val();
        if (!isNumeric(currentValue)) {
            // Display an error message to the user
            alert("Please enter a numeric value for the bank account number.");

            // Revert to the last known good value
            $(this).val($(this).data('original-value'));
        } else {
            // Update the stored 'original-value' since this is a valid numeric input
