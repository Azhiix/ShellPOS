...
window.onload = function () {
    ...
    document.getElementById('uploadForm').addEventListener('submit', function (event) {
        ...
        // If file size is okay, continue with your logic
        document.getElementById(employeeIdUploadClientId).value = document.querySelector('.clsempid').value;

        // Here you might want to handle the form submission via AJAX
        // or some other JavaScript method to avoid page reload
    });

    // Attach a change event listener to the file input
    document.getElementById('upload-btn').addEventListener('change', function () {
        // You can still keep this event to check file size when a file is selected
        ...
        success: function (response) {
            ...
            parsedRows.forEach(row => {
                ...
                table.row.add([
                    ...
                    /*"<a onclick='downloadDocument(\"" + row['uuid'] + "\",\"" + row['file_extension'] + "\")' class='btn btn-bg-secondary'>Download</a> <a class='btn btn-bg-primary'>View</a>" // Fourth Column HTML*/
                    "<a onclick='downloadDocument(\"" + row['uuid'] + "\",\"" + row['file_extension'] + "\")' class='btn btn-bg-secondary'>Download</a> <a class='btn btn-bg-danger'>Remove</a>" // Fourth Column HTML
                ]).draw(false);
            }
            );
