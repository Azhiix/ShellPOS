        ...
        success: function (successmsg) {
            ...
            $.each(successmsg.d, function (i, item) {
                ...
                if (!aggregatedData[key]) {
                    aggregatedData[key] = {
                        "templateKey": key,
                        "day": '<ul><li>' + findDayFromInteger(item.dayOfTheWeek) + '</li>',
                        "shiftStart": '<ul><li>' + formatTimeSpan(item.shiftStart) + '</li>',
                        "shiftEnd": '<ul><li>' + formatTimeSpan(item.shiftEnd) + '</li>',
                        "workingDay": item.workingDay ? '<ul><li>Yes</li>' : '<ul><li>No</li>',
                        "latenessTolerance": '<ul><li>' + item.latenessToleranceMinutes + ' min</li>',
                        "deleteButton": '<button type="button" class="btn btn-icon btn-danger delete-btn"><i class="bi bi-trash"></i></button>'
                    };
                } else {
                    aggregatedData[key].day += '<li>' + findDayFromInteger(item.dayOfTheWeek) + '</li>';
                    aggregatedData[key].shiftStart += '<li>' + formatTimeSpan(item.shiftStart) + '</li>';
                    aggregatedData[key].shiftEnd += '<li>' + formatTimeSpan(item.shiftEnd) + '</li>';
                    aggregatedData[key].workingDay += item.workingDay ? '<li>Yes</li>' : '<li>No</li>';
                    aggregatedData[key].latenessTolerance += '<li>' + item.latenessToleranceMinutes + ' min</li>';
                }
            });

            $.each(aggregatedData, function (i, aggregatedRow) {
                aggregatedRow.day += '</ul>';
                aggregatedRow.shiftStart += '</ul>';
                aggregatedRow.shiftEnd += '</ul>';
                aggregatedRow.workingDay += '</ul>';
                aggregatedRow.latenessTolerance += '</ul>';
