        ...
        success: function (successmsg) {
            ...
            $.each(successmsg.d, function (i, item) {
                ...
                if (!aggregatedData[key]) {
                    aggregatedData[key] = {
                        "templateKey": key,
                        "day": '<ul><li>' + findDayFromInteger(item.dayOfTheWeek),
                        "shiftStart": formatTimeSpan(item.shiftStart),
                        "shiftEnd": formatTimeSpan(item.shiftEnd),
                        "workingDay": item.workingDay ? 'Yes' : 'No',
                        "latenessTolerance": item.latenessToleranceMinutes + ' min',
                        "deleteButton": '<button type="button" class="btn btn-icon btn-danger delete-btn"><i class="bi bi-trash"></i></button>'
                    };
                } else {
                    aggregatedData[key].day += ', ' + findDayFromInteger(item.dayOfTheWeek);
                    aggregatedData[key].shiftStart += ', ' + formatTimeSpan(item.shiftStart);
                    aggregatedData[key].shiftEnd += ', ' + formatTimeSpan(item.shiftEnd);
                    // Assuming workingDay and latenessTolerance are the same for all entries with the same templateKey
                }
            });

            $.each(aggregatedData, function (i, aggregatedRow) {
                aggregatedRow.day += '</li></ul>'; // Close the list item and unordered list tags
