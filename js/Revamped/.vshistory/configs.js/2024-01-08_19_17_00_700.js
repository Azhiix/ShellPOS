...
function createShift() {
    ...
    for (var i = 0; i < daysOfWeek.length; i++) {
        ...
        var shiftStart = $('#' + day + 'WorkingDay').is(':checked') ? $('#' + day + 'StartTime').val() : 0;
        var shiftEnd = $('#' + day + 'WorkingDay').is(':checked') ? $('#' + day + 'EndTime').val() : 0;

        if (shiftStart !== 0 && shiftEnd === 0) {
            alert('Please specify a proper start/end time for ' + day.charAt(0).toUpperCase() + day.slice(1) + '.');
            return; // Stop the function execution if the end time is missing
        }
