...
function initFilter() {
    $('#employeeStatus').val('1').trigger('click');
...
function filterStatus(employeeStatus) {
    ...
    // Custom search function to filter based on the hidden includedInPayroll value
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        // Ensure this custom filter is only applied to your specific table
        if (settings.nTable.id !== 'setEmpLandTable') {
            return true;
        }

        // Access the row and find the included-in-payroll div
        var row = table.row(dataIndex).node();
        var includedInPayroll = $(row).find('.included-in-payroll').text().trim();

        // Compare the text content of the div with the employeeStatus
        return includedInPayroll === employeeStatus;
    });

    // Redraw the table to apply the new search
    table.draw();

    // Remove the added search function to not affect global search behaviour
