        ...
        success: function (response) {
            ...
            parsedRows.forEach(row => {
                ...
                table.row.add([
                    ...
                    "<a onclick='downloadDocument(\"" + row['uuid'] + "\",\"" + row['file_extension'] + "\")' class='btn btn-bg-secondary'>Download</a> <a onclick='removeDocument(\"" + row['uuid'] + "\")' class='btn btn-bg-danger'>Remove</a>" // Fourth Column HTML
                ]).draw(false);
            }
            );
...
function removeDocument(uuid) {
    ...
    $.ajax({
        type: "POST",
        url: "YourPage.aspx/RemoveEmployeeDocument", // Replace with the actual path to your ASPX page
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ employeeId: employeeId, uuid: uuid }),
        success: function (response) {
