...
$(document).ready(function () {
    ...
    $('.select2-branches').on('change', function () {
        // Close any open collapse panels
        $('.collapse').collapse('hide');

        // Get the selected branch id
        var branchId = $(this).val();

        // Open the collapse panel for the selected branch
        if (branchId) {
...
function loadCompDetails() {
    ...
    //$(".mainSettingsContainer").css("display", "none");
    //$(".compDetails").css("display", "grid");
...
function updateCompDetails() {
    ...
    if (companyName === "" || compAddress === "" || BRN === "" || VAT === "" || BPAcNumber === "" || BPAcType === "" || BPText === "") {
    ...
    if (boolCorrectData == 1) {
        ...
        jQuery.ajax({
            type: "POST",
            url: "settings.aspx/updateCompDetails", //It calls our web method  
            data: JSON.stringify({
...
function loadCompDetailsFromDB() {
    //here we retrieve the company details from the database
    jQuery.ajax({
        type: "POST",
        ...
        async: false,
        success: function (msg) {
            sessionStorage.setItem('companyName', msg.d.companyName);
            sessionStorage.setItem('CompAddress', msg.d.compAddress);
            sessionStorage.setItem('BRN', msg.d.BRN);
            sessionStorage.setItem('VAT', msg.d.VAT);
            sessionStorage.setItem('bulkPaymentID', msg.d.bulkPaymentID);
            sessionStorage.setItem('BPAcNumber', msg.d.BPAcNumber);
            sessionStorage.setItem('BPAcType', msg.d.BPAcType);
            sessionStorage.setItem('BPText', msg.d.BPText);
            sessionStorage.setItem('email', msg.d.email);
            sessionStorage.setItem('telNum', msg.d.telNum);
            sessionStorage.setItem('faxNum', msg.d.faxNum);
            sessionStorage.setItem('startDate', msg.d.shiftDetails.startDate);
            sessionStorage.setItem('startMonth', msg.d.shiftDetails.startMonth);
            sessionStorage.setItem('endDate', msg.d.shiftDetails.endDate);
            sessionStorage.setItem('latenessThreshold', msg.d.shiftDetails.latenessThreshold);
            sessionStorage.setItem('latenessThresholdWE', msg.d.shiftDetails.latenessThresholdWE);
            sessionStorage.setItem('perfDeduct', msg.d.shiftDetails.perfDeduct);
            sessionStorage.setItem('attDeduct', msg.d.shiftDetails.attDeduct);
            sessionStorage.setItem('WDST', msg.d.shiftDetails.WDST);
            sessionStorage.setItem('WDET', msg.d.shiftDetails.WDET);
            sessionStorage.setItem('WEST', msg.d.shiftDetails.WEST);
            sessionStorage.setItem('WEET', msg.d.shiftDetails.WEET);
            sessionStorage.setItem('CLEL', msg.d.shiftDetails.CLEL);
            sessionStorage.setItem('COAR', msg.d.shiftDetails.COAR);
            sessionStorage.setItem('COLL', msg.d.shiftDetails.COLL);
...
function bindTimesheetDefaultsButtons() {
    $('#toggleCLEL').on('change', function (event) {
        handleTimesheetToggle(this, 'CLEL', 'configs.aspx/UpdateStatus');
    });
    $('#toggleCOLL').on('change', function (event) {
        handleTimesheetToggle(this, 'COLL', 'configs.aspx/UpdateStatus');
    });
    $('#toggleCOAR').on('change', function (event) {
        handleTimesheetToggle(this, 'COAR', 'configs.aspx/UpdateStatus');
    });
    $('#togglecsgeligible').on('change', function (event) {
        handleTimesheetToggle(this, 'csgeligible', 'configs.aspx/UpdateStatus');
    });
};

function handleTimesheetToggle(checkbox, toggleId, updateUrl) {
    ...
    jQuery.ajax({
        ...
        success: function (msg) {
            if (msg.d == null) {
                ...
                alert(toggleId + ' status successfully updated');
            } else {
                alert('Error while updating ' + toggleId + '. Please contact your administrator.');
            }
        },
...
function setToggleStatus(statusData) {
    for (var toggleId in statusData) {
        var isChecked = statusData[toggleId];
        $('#toggle' + toggleId).prop('checked', isChecked);
    }
}
