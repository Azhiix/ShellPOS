...
function initFilter() {
    ...
    // Trigger the click event on page load to apply the filter with the default value
    $('#apply-filter').click();
    ...
    $('#reset-filter').on('click', function () {
        ...
        $('.empctrl').val(null).trigger('change');
        $('.deptctrl').val(null).trigger('change');
    });
}
...
function applyFilters() {
    ...
    if (filterState.status) {
        // Search for 'includedInPayroll' in the 5th column
        // We are using regex to search within the hidden div
        table.column(4).search('<div class="included-in-payroll" style="display: none;">' + filterState.status + '</div>', true, false).draw();
    }
    if (filterState.employeeId) {
        // Search for 'employeeId' in the 5th column
        // We are using regex to search within the hidden div
        table.column(4).search('<div class="employeeId" style="display: none;">' + filterState.employeeId + '</div>', true, false).draw();
    }
    if (filterState.department) {
        // Search for department in the 4th column
        table.column(3).search(filterState.department).draw();
    }
}
...
function filterStatus(employeeStatus) {
    var table = $('#setEmpLandTable').DataTable();
...
function initDeptCtrlSelect2() {
    $('.deptctrl').on('change', function () {
        ...
        if (selectedDept) {
            table.column(3).search(selectedDept).draw();
        } else {
...
function initDataTable(lstEmp, lstDept) {
    ...
    lstEmp.forEach(function (emp) {
        ...
        table.row.add([
            ...
            includedInPayrollDiv + employeeIdDiv + '<a href="#" class="btn btn-bg-secondary" onclick="loadPersonalEmpDetails(' + emp.emp_pin + ')">View</a>'
        ]).draw(false);
    });
...
function LoadSalaryDetails(salaryDetails) {
    ...
    // Sort the salaryDetails array by orderID
    ...
    $.each(salaryDetails, function (index, item) {
        // Alert 'Hello world' if orderID is 0
        if (item.orderID == 0) {
            ...
            // Use the orderID instead of the index for class selectors
            $("." + item.orderID + "Name").val(item.salaryDetailName);
            $("." + item.orderID + "Amt").val(item.salaryDetailAmount);
            $("." + item.orderID + "RemPolicy").val(item.calculationBasisID).trigger('change');
            $("." + item.orderID + "PolicyValue").val(item.calculationBasisValue);
            $("." + item.orderID + "PolicyValueTime").val(convertJsonDateToTime(item.calculationBasisTime));
            remPolicyChangedFnctn("." + item.orderID + "RemPolicy");
...
function postChangesToEmployee() {
    ...
    $('.addItemsInPayroll .row').each(function () {
        ...
        if (!salItemName || !salItemAmount || !remunPolicy) {
            alert('Please check whether the salary values are not empty');
            allDataIsValid = false;
            return; // Skip the current iteration
        }

        if((!(remunPolicy == '2' || remunPolicy == '1')) && (policyVal == null || policyVal == ''))
        {    alert('Please check whether the salary values are not empty'); 
            allDataIsValid = false;
            return; // Skip the current iteration
        }
    ...
    if (allDataIsValid) {
        ...
        var payrollItem = new PayrollItem(emp_pin, "Basic Salary", basicSalary, 0, 0, "07:00", 0);
        payrollItems.push(payrollItem);
