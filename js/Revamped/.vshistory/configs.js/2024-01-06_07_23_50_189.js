            ...
            $.each(successmsg.d, function (i, item) {
                ...
                if (!aggregatedData[key]) {
                    aggregatedData[key] = {
                        ...
                        "shiftStart": formatTimeSpan(item.shiftStart),
                        "shiftEnd": formatTimeSpan(item.shiftEnd) + '</li>',
                        "workingDay": item.workingDay ? 'Yes' : 'No',
                        "latenessTolerance": item.latenessToleranceMinutes,
                    ...
                    aggregatedData[key].shiftStart += formatTimeSpan(item.shiftStart);
                    aggregatedData[key].shiftEnd +=formatTimeSpan(item.shiftEnd);
                    aggregatedData[key].workingDay += item.workingDay ? 'Yes' : 'No';
                    aggregatedData[key].latenessTolerance +=  item.latenessToleranceMinutes + ' min';
                }
            });
