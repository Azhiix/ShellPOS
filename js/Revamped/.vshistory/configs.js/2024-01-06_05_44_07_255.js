...
function loadShift() {
    ...
    jQuery.ajax({
        ...
        success: function (successmsg) {
            ...
            for (var x = 0; x < successmsg.d.length; x++) {
                var dateReceived = days[ToJavaScriptDateTime(successmsg.d[x].dateReceived).getDay()] + " " + ToJavaScriptDateTime(successmsg.d[x].dateReceived).getDate() + " " + months[ToJavaScriptDateTime(successmsg.d[x].dateReceived).getMonth()] + " " + ToJavaScriptDateTime(successmsg.d[x].dateReceived).getFullYear();
                var startMonth = months[ToJavaScriptDateTime(successmsg.d[x].firstMonthDeduct).getMonth()] + " " + ToJavaScriptDateTime(successmsg.d[x].firstMonthDeduct).getFullYear();
                var endMonth = months[ToJavaScriptDateTime(successmsg.d[x].lastMonthDeduct).getMonth()] + " " + ToJavaScriptDateTime(successmsg.d[x].lastMonthDeduct).getFullYear();
                $("#tblContent").append("<tr><td>" + successmsg.d[x].emp_name + "</td><td>" + successmsg.d[x].amtTaken + "</td><td>" + dateReceived + "</td><td>" + successmsg.d[x].installments + "</td><td>" + startMonth + "</td><td>" + endMonth + "</td><td><input type='checkbox' value='" + successmsg.d[x].id + "' />&nbsp;</td></tr >");
            }
        },
        failure: function (msg) {
            alert(msg);
