...
function filterStatus(employeeStatus) {
    ...
    // Extend the default search function
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        // Check if we are working on the correct table
        if (settings.nTable.id !== 'setEmpLandTable') {
            return true; // If not, do not filter
        }

        // Get the row and find the 'included-in-payroll' div
        var row = table.row(dataIndex).node();
        var includedInPayroll = $(row).find('.included-in-payroll').text().trim();

        // Check if the includedInPayroll matches the employeeStatus
        if (includedInPayroll === employeeStatus) {
