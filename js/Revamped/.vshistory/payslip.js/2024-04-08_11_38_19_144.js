...
async function printAllPayslips() {
    ...
    // Fetch all payslip HTML content asynchronously
    var payslipsHtml = await getAllPayslipHtml();
...
async function getAllPayslipHtml() {
    ...
    try {
        const response = await fetch('payslips.aspx/loadPayslips', {
            ...
            // Adjust this payload as needed to fetch multiple payslips
            body: JSON.stringify({ /* your payload here */ }),
        });
        const data = await response.json();

        if (data && data.d) {
            data.d.forEach(payslipData => {
                const payslipHtml = generatePayslipHTML(payslipData, payslipData);
