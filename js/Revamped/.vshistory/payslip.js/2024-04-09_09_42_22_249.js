...
async function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(async allPayslipHtmlContents => {
        var doc = new jsPDF({
            orientation: 'portrait',
            unit: 'mm',
            format: 'a5'
        ...
        for (let i = 0; i < allPayslipHtmlContents.length; i++) {
            if (i > 0) doc.addPage(); // Add a new page for each payslip after the first one

            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = allPayslipHtmlContents[i];
            ...
            doc.addImage(imgData, 'JPEG', 10, 10, 190, 280); // Adjust dimensions as needed

            document.body.removeChild(tempDiv); // Clean up by removing the temp div
        }

        doc.save('payslips.pdf'); // Save the PDF after all payslips have been processed
    });
}