...
function addPayslipsToHTML(payslipHtmlContent) {
    ...
    var payslips = payslipHtmlContent.split('<div class="pagebreak"></div>');

    // Append each payslip with a page break
    payslips.forEach(function (payslipHtml, index) {
        // For each payslip, append it to the new container
        newContainer.append(payslipHtml);

        if (index !== payslips.length - 1) {
            // Append a page break after each payslip except the last one
            newContainer.append('<div style="page-break-after: always" class="pagebreak"></div>');
        }
    });

    // Set the HTML of the payslipContainer to the HTML of the new container
    payslipContainer.html(newContainer.html());
