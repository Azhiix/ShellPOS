...
function printAllPayslips() {
    var payslipPromises = $('.payslipTblBody tr').map(function () {
        ...
        return fetch("payslips.aspx/loadPayslip", {
            ...
            .catch(error => {
                ...
                return `<div>Error loading payslip for ID ${payslipId}</div>`;
            });
    }).get();
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        var addPayslipToPDF = function (index) {
            if (index >= allPayslipHtmlContents.length) {
                doc.save('payslips.pdf'); // Save the PDF when all payslips have been added
                return; // Exit the function
            }

            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = allPayslipHtmlContents[index];
            document.body.appendChild(tempDiv); // Temporarily add to the document to render

            // Convert the HTML to a canvas
            html2canvas(tempDiv).then(canvas => {
                document.body.removeChild(tempDiv); // Clean up by removing the temp div

                // Adjust dimensions as needed to fit A5 and maintain aspect ratio
                ...
                addPayslipToPDF(index + 1); // Process the next payslip
            });
        };

        addPayslipToPDF(0); // Start the process with the first payslip
    });
}