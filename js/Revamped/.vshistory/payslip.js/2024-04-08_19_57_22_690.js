...
function generatePDFA5() {
    ...
    // Select all payslip elements
    const payslips = document.querySelectorAll('.payslip');
    const payslipCount = payslips.length;
    let currentPayslip = 0;

    // Function to process each payslip
    function processPayslip(payslipElement) {
        html2canvas(payslipElement, { scale: 3 }).then(canvas => {
            ...
            // Add image and possibly add new pages if the content overflows
            let position = 0; // Initial position at the top of the page

            pdf.addImage(imgData, 'PNG', 0, position, imgWidth, imgHeight);
            heightLeft -= pageHeight;

            while (heightLeft >= 0) {
                position = heightLeft - imgHeight; // Top position of the next image
                pdf.addPage();
                pdf.addImage(imgData, 'PNG', 0, position, imgWidth, imgHeight);
                heightLeft -= pageHeight;
            }

            // Move to the next payslip or save the PDF if done
            currentPayslip++;
            if (currentPayslip < payslipCount) {
                pdf.addPage(); // Ensure each payslip starts on a new page
                processPayslip(payslips[currentPayslip]);
            } else {
                // Save the PDF once all payslips have been processed
                pdf.save('payslips.pdf');
            }
        });
    }

    // Start processing the first payslip
    if (payslipCount > 0) {
        processPayslip(payslips[currentPayslip]);
    }
}
