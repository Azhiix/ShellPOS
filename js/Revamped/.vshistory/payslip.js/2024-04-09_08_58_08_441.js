...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        // Function to render each HTML content to a PDF page
        function renderPayslipsToPDF(allPayslipHtmlContents) {
            // Create an instance of jsPDF
            const pdf = new jspdf.jsPDF();

            allPayslipHtmlContents.forEach((content, index) => {
                ...
                html2canvas(tempDiv[0]).then(canvas => {
                    ...
                    img.onload = function () {
                        ...
                        pdf.addImage(this, 'PNG', 0, 0, pdf.internal.pageSize.getWidth(), pdf.internal.pageSize.getHeight());
                        if (index === allPayslipHtmlContents.length - 1) {
                            pdf.save('payslips.pdf');
                        }
                    };
                    img.src = imageDataUrl; // This should start the img.onload process
                    tempDiv.remove();
                });
            });
        }
        ...
        // Call function
        renderPayslipsToPDF(allPayslipHtmlContents);
    });
}
