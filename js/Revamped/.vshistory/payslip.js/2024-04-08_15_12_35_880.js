...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        tempContainer.style.visibility = 'hidden'; // Make it not visible, but still renderable
        tempContainer.innerHTML = finalHtmlContent;
        ...
        html2canvas(tempContainer).then(canvas => {
            ...
            // Convert the canvas to an image and use it in addPayslipsToHTML
            var imgData = canvas.toDataURL("image/png");
            var imgTag = '<img src="' + imgData + '">';
            addPayslipsToHTML(imgTag);
        });
    });
