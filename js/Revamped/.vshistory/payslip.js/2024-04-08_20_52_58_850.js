...
function addPayslipsToHTML(payslipHtmlContent) {
    ...
    // Clear existing content in the payslip container
    payslipContainer.empty();

    // Split the payslipHtmlContent by the 'pagebreak' divs to get individual payslips
    ...
    payslips.forEach(function (payslipHtml, index) {
        // Create a new div for this payslip
        var payslipDiv = $('<div></div>');
        payslipDiv.html(payslipHtml);

        // Append this payslip to the container
        payslipContainer.append(payslipDiv);

        // If this isn't the last payslip, add a CSS-based page break after it
        if (index !== payslips.length - 1) {
            payslipContainer.append('<div style="page-break-after: always;"></div>');
        }
    });

    // Now that all payslips and necessary page breaks are appended,
    // you can proceed with the next steps, like printing or generating a PDF
    // Example: Trigger printing
    // window.print();

    // Or if you are generating a PDF, call that function here
