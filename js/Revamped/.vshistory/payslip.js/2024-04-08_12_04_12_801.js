        ...
        jQuery.ajax({
            ...
            success: function (msg) {
                if (msg.d == null) {
                    ...
                    console.log("first console log is", msg.d);
    ...
    $('#idPs2Table').on('click', '.viewPayslip', function () {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                console.log( "2nd console log is", payslipData)
                ...
                generatePDFA5(res);
                console.log("3rd console log is", res);
...
function fetchPayslipData(payslipId) {
    return new Promise((resolve, reject) => {
        jQuery.ajax({
            ...
            dataType: "JSON",
            success: function (response) {
                resolve(response.d); // Resolve with the data on success
            },
            error: function (xhr, status, error) {
...
function printAllPayslips(payslipIds) {
    // Map each payslipId to a fetchPayslipData promise
    const fetchPromises = payslipIds.map(payslipId => fetchPayslipData(payslipId));

    // Use Promise.all to wait for all AJAX calls to complete
    Promise.all(fetchPromises)
        .then(allPayslipsData => {
            // Generate HTML for all payslips
            const allPayslipsHtml = getAllPayslipHtml(allPayslipsData);
            // Implement the printing of allPayslipsHtml as before
            ...
            iframeDoc.open();
            iframeDoc.write(allPayslipsHtml);
            iframeDoc.close();
...
function getAllPayslipHtml(allPayslipsData) {
    allPayslipsData.forEach(payslipData => {
        const payslipHtml = generatePayslipHTML(payslipData.payslip, payslipData);
        allPayslipsHtml += payslipHtml;
    });
