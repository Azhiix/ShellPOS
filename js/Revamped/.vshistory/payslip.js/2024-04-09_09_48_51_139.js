...
function printAllPayslips() {
    var payslipPromises = $('.payslipTblBody tr').map(function () {
        ...
        return fetch("payslips.aspx/loadPayslip", {
            ...
            .then(data => {
                ...
                return generatePayslipHTML(multiResult.payslip, multiResult);
            })
            .catch(error => {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        allPayslipHtmlContents.forEach((htmlContent, index) => {
            const tempDiv = document.createElement('div');
            ...
            html2canvas(tempDiv, { scale: 1.5 }).then(canvas => {
                ...
                const imgData = canvas.toDataURL('image/png');
                const contentStartY = 5;
                const pageWidth = pdf.internal.pageSize.getWidth() - 2 * contentStartY;
                const imgHeight = canvas.height * pageWidth / canvas.width;

                pdf.addImage(imgData, 'PNG', contentStartY, contentStartY, pageWidth, imgHeight);
