...
function generatePayslipHTML(payslipData, multires) {
    ...
    initialHtml += `<div class='row text-left'><div class='col-12'>Local Leaves: ${absencesTaken.localTaken}</div></div>`;
    initialHtml += `<div class='row text-left'><div class='col-12'>Sick Leaves: ${absencesTaken.sickTaken}</div></div>`;
    initialHtml += `<div class='row text-left'><div class='col-12'>Leaves Without Pay: ${absencesTaken.leavesWithoutPayTaken}</div></div>`;
    initialHtml += `<div class='row text-left'><div class='col-12'>Paid Leaves: ${absencesTaken.paidLeavesTaken}</div></div>`;
...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        let isFirstPayslip = true;

        allPayslipHtmlContents.forEach((payslipHtml, index) => {
            if (!isFirstPayslip) {
                // Add a new page for every payslip after the first
                pdf.addPage();
            }
            isFirstPayslip = false;

            // Convert the current payslip HTML to canvas then to an image for PDF
            let tempDiv = document.createElement('div');
            tempDiv.innerHTML = payslipHtml;
            document.body.appendChild(tempDiv); // Temporarily add to DOM to measure

            html2canvas(tempDiv, { scale: 1 }).then(canvas => {
                const imgData = canvas.toDataURL('image/png');

                pdf.addImage(imgData, 'PNG', 0, 0, 210, canvas.height * 210 / canvas.width, null, 'FAST'); // Adjust as necessary

                document.body.removeChild(tempDiv); // Cleanup

                if (index === allPayslipHtmlContents.length - 1) { // Check if it's the last payslip
