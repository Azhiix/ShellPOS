...
function generatePDFA5(htmlString) {
    ...
    html2canvas(tempDiv, { scale: 1 }).then(canvas => {
        ...
        const pageHeight = pdf.internal.pageSize.getHeight() - 2 * contentStartY;
        const imgHeight = canvas.height * ((pdf.internal.pageSize.getWidth() - 2 * contentStartY) / canvas.width);
        let heightLeft = imgHeight;

        let position = 0;

        pdf.addImage(imgData, 'PNG', contentStartY, contentStartY, pdf.internal.pageSize.getWidth() - 2 * contentStartY, imgHeight);
        ...
        while (heightLeft >= 0) {
            ...
            pdf.addPage();
            pdf.addImage(imgData, 'PNG', contentStartY, position, pdf.internal.pageSize.getWidth() - 2 * contentStartY, imgHeight);
            heightLeft -= pageHeight;
        }
        ...
        pdf.text(disclaimerText, textStartX, disclaimerY);
...
function DownloadPDFA5(htmlString, emp_name) {
    ...
    html2canvas(tempDiv, { scale: 1.5 }).then(canvas => {
        ...
        const disclaimerY = pdf.internal.pageSize.getHeight() - 7; // 10mm from the bottom
...
function generatePayslipHTML(payslipData, multires) {
    ...
    var initialHtml = "<div ></div><div  emp_pin='" + payslipData.emp_pin + "'>";
    ...
    // Additional HTML
    initialHtml += `<div class='row'><div class='col-12 d-flex justify-content-center'> <img src='${logoSource}' class='customLogoSize' /> </div></div>`
    ...
    initialHtml += `<div class='row text-left'>
<div class='col-6'>Local Leaves: ${absencesTaken.localTaken}</div>
<div class='col-6'>Sick Leaves: ${absencesTaken.sickTaken}</div>
<hr/>
</div>`;
    initialHtml += `<div class='row text-left'>
<div class='col-6'>Leaves Without Pay: ${absencesTaken.leavesWithoutPayTaken}</div>
<div class='col-6'>Paid Leaves: ${absencesTaken.paidLeavesTaken}</div>
</div>`;
...
function addPayslipsToHTML(payslipHtmlContent) {
    var payslipContainer = $("#payslipContainer");
    payslipContainer.html(payslipHtmlContent);

    // Split the payslipHtmlContent by a custom divider if you have one or ensure each payslip is wrapped in a div that can be identified
    var payslips = payslipHtmlContent.split('<div class="pagebreak"></div>'); // Assuming payslipHtmlContent is one big string of HTML

    // Append each payslip with a page break
    payslips.forEach(function (payslipHtml, index) {
        // For each payslip, append it to the container
        payslipContainer.append(payslipHtml);


        if (index !== payslips.length - 1) {
            payslipContainer.append('<div style="page-break-after: always;"></div>');
        }
    });

    // Once all payslips are appended with necessary page breaks, you can trigger printing or whatever next steps you have
    // For example, if you are generating a PDF, you would then call your function to convert this HTML to PDF here
    generatePDFA5(payslipContainer.html());
}

function printAllPayslips() {
    // Mapping each table row to a promise that resolves with generated HTML content for the payslip
    var payslipPromises = $('.payslipTblBody tr').map(function () {
        var payslipId = $(this).data('payslip-id');
        var jwtToken = getCookie("jwttoken");

        console.log(`Fetching payslip for ID: ${payslipId}`);
        // Fetching the payslip data
        return fetch("payslips.aspx/loadPayslip", {
            method: "POST",
            headers: {
                'Authorization': 'Bearer ' + jwtToken,
                'Content-Type': "application/json; charset=utf-8"
            },
            body: JSON.stringify({ payslipId: payslipId })
        })
            .then(response => response.json())
            .then(data => {
                console.log(`Processing payslip data for ID: ${payslipId}`);
                var multiResult = data.d;
                const payslipData = multiResult.payslip;

                return generatePayslipHTML(payslipData, multiResult);
            })
            .catch(error => {
                console.error(`Error occurred fetching or processing payslip for ID ${payslipId}:`, error);
                return `<div>Error loading payslip for ID ${payslipId}. Details: ${error}</div>`;
            });
    }).get();

    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        console.log('All payslip data fetched and processed. Rendering to PDF.');
        // Create an instance of jsPDF
        const pdf = new jspdf.jsPDF();

        let canvasPromises = allPayslipHtmlContents.map(content => {
            const tempDiv = $('<div>').appendTo('body').css({ display: 'none' }).html(content);
            // Setting html2canvas logging to true for better debugging
            return html2canvas(tempDiv[0], { logging: true, useCORS: true }).then(canvas => {
                tempDiv.remove(); // Ensures clean-up after conversion
                return canvas.toDataURL('image/png');
            }).catch(error => {
                console.error('Error during canvas conversion:', error);
                return null; // Handle cases where canvas conversion fails
            });
        });
        ...
        Promise.all(canvasPromises).then(images => {
            images.forEach((imgDataUrl, index) => {
                if (!imgDataUrl) {
                    console.error(`Skipping a payslip due to conversion error. Index: ${index}`);
                    return; // Skips pages that failed to convert properly
                }
                ...
                try {
                    ...
                    console.error(`Error adding image to PDF for index ${index}:`, error);
                }
            });
            ...
            if (images.filter(Boolean).length > 0) {
                ...
                console.error('No payslips were processed successfully.');
            }
        });
    });
}