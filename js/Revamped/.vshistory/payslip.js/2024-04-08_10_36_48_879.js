        ...
        jQuery.ajax({
            ...
            success: function (msg) {
                if (msg.d == null) {
                    ...
                    var payslipsContainer = $('#payslipsContainer');
                    payslipsContainer.empty(); // Clear previous contents

                    // Iterate over each payslip data in msg.d
                    msg.d.forEach(function (payslip) {
                        // Assuming each 'payslip' object has the necessary data fields
                        // Generate the HTML string for this payslip
                        var payslipHTML = `<div class="payslip">
                <h3>Payslip for ${payslip.emp_name}</h3>
                <p>Period: ${payslip.period}</p>
                <p>Net Salary: ${payslip.netSalary}</p>
                <!-- Add more details as needed -->
            </div>`;
                        payslipsContainer.append(payslipHTML);
                    })
...
function printAllPayslips(payslipData) {
    ...
    var payslipsHtml = getAllPayslipHtml(payslipData);
...
function getAllPayslipHtml(payslipData) {
    var allPayslipsHtml = "<html><head><title>All Your Payslips</title></head><body>";
    allPayslipsHtml += "<h1>Hello</h1><p>Here are all your payslips:</p>";

    // Loop through each payslip and generate HTML
    payslipData.forEach(function (payslip) {
        // Example of generating simple payslip HTML
        allPayslipsHtml += `<div>
            <h2>Payslip for ${payslip.emp_name}</h2>
            <p>Period: ${payslip.period}</p>
            <p>Net Salary: ${payslip.netSalary}</p>
            <!-- Include other payslip details as needed -->
        </div>`;
    });
