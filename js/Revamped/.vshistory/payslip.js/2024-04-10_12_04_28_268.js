...
function initEvents() {
    ...
    $('#idPs2Table').on('click', '.downloadPayslip', async function () {
        var payslipId = $(this).attr('data-payslip-id');

        // Convert single payslipId to array because loadPayslips expects an array of IDs
        var payslipIds = [payslipId];

        try {
            // Immediately Invoked Function Expression (IIFE) to use async/await
            (async () => {
                var response = await loadPayslips(payslipIds);

                // Assuming the response contains the data in a similar structure to what you expect
                var payslipData = response.payslip; // Adjust according to actual structure

                // Proceed with your logic to generate and download the payslip PDF
                const doc = new jsPDF();
                var res = generatePayslipHTML(payslipData, response); // Adjust according to actual structure
                $('#payslipContainer').html(res);
