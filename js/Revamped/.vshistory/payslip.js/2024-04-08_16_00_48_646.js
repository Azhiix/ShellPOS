...
function addPayslipsToHTML(payslipHtmlContent) {
    // Append the payslip HTML content to a designated container
    var payslipContainer = document.getElementById('payslipContainer');
    if (!payslipContainer) {
        payslipContainer = document.createElement('div');
        payslipContainer.id = 'payslipContainer';
        document.body.appendChild(payslipContainer);
    }
    payslipContainer.innerHTML = payslipHtmlContent; // Set or append the HTML content

    // Use html2canvas to render the payslipContainer content onto a canvas
    html2canvas(payslipContainer, {
        onrendered: function (canvas) {
            // You can do various things with the canvas
            // For example, append it to the body to visually debug or inspect
            document.body.appendChild(canvas);

            // If you want to directly print the canvas, you could convert it to an image and then print
            // Or you might open a new window/tab with the canvas drawn to trigger a print dialog
        }
    });
}
