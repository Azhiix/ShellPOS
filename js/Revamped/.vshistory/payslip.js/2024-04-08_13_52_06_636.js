...
var printingInProgress = false; // Flag to track printing status

function addPayslipsToHTML(payslipHtmlContent) {
    if (printingInProgress) {
        // Printing is already in progress, do not initiate another print request
        return;
    }
    ...
    printingInProgress = true; // Set flag to indicate printing is in progress

    // Trigger print after appending content
    iframe.contentWindow.focus(); // Ensure the iframe is focused
    iframe.contentWindow.print(); // Trigger the print dialog

    // Reset flag after print completes (assuming iframe.onload event doesn't fire reliably)
    setTimeout(function () {
...
function printAllPayslips() {
    var payslipIds = $('.payslipTblBody tr').map(function () {
        return $(this).data('payslip-id');
    }).get();

    var jwtToken = getCookie("jwttoken");
    var payslipRequests = payslipIds.map(payslipId => {
        return new Promise((resolve, reject) => {
            $.ajax({
                type: "POST",
                url: "payslips.aspx/loadPayslip",
                data: JSON.stringify({ payslipId: payslipId }),
                headers: {
                    'Authorization': 'Bearer ' + jwtToken
                },
    ...
    Promise.all(payslipRequests)
        .then(responses => {
            responses.forEach(response => {
                var multiResult = response.d;
                var payslipData = multiResult.payslip;
                var res = generatePayslipHTML(payslipData, multiResult); // Ensure this function is prepared to handle your data structure
                addPayslipsToHTML(res);
            });
        })
        .catch(error => {
            console.error("Error occurred with one of the payslip requests: ", error);
