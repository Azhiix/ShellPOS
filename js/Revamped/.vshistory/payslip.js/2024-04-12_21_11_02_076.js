...
async function loadPayslips(payslipIds) {
        try {
            const jwtToken = getCookie("jwttoken"); // Assuming getCookie function is defined elsewhere

            const response = await fetch("payslips.aspx/LoadPayslips", {
                method: "POST",
                headers: {
...
function initEvents() {
  ...
  $('#idPs2Table').on('click', '.viewPayslip', async function (e) {
    e.preventDefault(); 
    var payslipId = $(this).attr('data-payslip-id');

    try {
        var multiResult = await loadPayslips([payslipId]); 
        console.log('Multi result is',multiResult)
        const payslipData = multiResult[0].payslip; 
    
        if (!payslipData) {
            throw new Error('Payslip data is undefined');
        }
    
        console.log('Payslip Data is', payslipData)
    
        const doc = new jsPDF();
        const empName = payslipData.emp_name;
        console.log('Emp Name is', empName)
    
        var res = generatePayslipHTML(payslipData, multiResult[0]);
        console.log(res)
        $('#payslipContainer').html(res);
        generatePDFA5(res);

        // Call the DownloadPDFA5 function with the HTML string and employee name
...
$('#idPs2Table').on('click', '.downloadPayslip', async function(e) {
...
function printAllPayslips() {
    ...
    var payslipIds = [];
    $('.payslipTblBody tr').each(function () {
        payslipIds.push($(this).data('payslip-id'));
    });

    loadPayslips(payslipIds).then(data => {
       console.log(data[0])
      

        var allPayslipsHTML = [];
        for (var i = 0; i < data.length; i++) {
            var payslipData = data[i].payslip;
            var res = generatePayslipHTML(payslipData, data[i]);
            allPayslipsHTML.push(res);
        }
        ...
        let totalCount = allPayslipsHTML.length;
        let processedCount = 0;

        allPayslipsHTML.forEach((htmlContent, index) => {
            const tempDiv = document.createElement('div');
