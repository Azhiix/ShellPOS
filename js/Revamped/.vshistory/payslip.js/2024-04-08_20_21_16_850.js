...
function addPayslipsToHTML(payslipHtmlContent) {
    // Identify or create a modal/dedicated section for the payslip preview
    var payslipPreviewModal = $("#payslipPreviewModal");
    var payslipPreviewContainer = payslipPreviewModal.find(".modal-body");

    // Clear existing content in the preview container
    payslipPreviewContainer.html('');

    // Split the payslipHtmlContent by your custom divider
    var payslips = payslipHtmlContent.split('<div class="pagebreak"></div>');

    // Append each payslip with a page break (for PDF conversion) or as separate pages (for modal preview)
    payslips.forEach(function (payslipHtml, index) {
        payslipPreviewContainer.append(payslipHtml);

        // For modal preview, consider not adding a physical page break but separating them visually
        if (index !== payslips.length - 1) {
            payslipPreviewContainer.append('<div class="payslip-separator"></div>');
        }
    });

    // Trigger the modal to open, showing the preview
