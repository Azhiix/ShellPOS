        ...
        jQuery.ajax({
            ...
            success: function (msg) {
                if (msg.d == null) {
                    ...
                    console.log(msg.d);
                    msg.d.forEach(message => {
                        console.log(message.emp_pin)
...
function printAllPayslips(allPayslipData) {
    ...
    var payslipsHtml = getAllPayslipHtml(allPayslipData);
...
function getAllPayslipHtml(allPayslipData) {
    var allPayslipsHtml = "<html><head><title>All Your Payslips</title></head><body>";

    // Ensure allPayslipData is an array and not empty
    if (Array.isArray(allPayslipData) && allPayslipData.length > 0) {
        allPayslipData.forEach(payslipData => {
            // Here, you'd generate your payslip HTML
            // This assumes you have a `generatePayslipHTML` function that does this
            allPayslipsHtml += generatePayslipHTML(payslipData);
        });
    } else {
        // Handle the case where no payslip data is available
        allPayslipsHtml += "<p>No payslip data available.</p>";
    }
...
function generatePayslipHTML(payslipData) {
    // Your logic to generate HTML for each payslip
    return "<p>Payslip for " + payslipData.employeeName + "</p>";
}
...
function getAllPayslipHtml(allPayslipData) {
    ...
    var allPayslipData = [];
    var allPayslipsHtml = "<html><head><title>All Your Payslips</title></head><body>";
    printAllPayslips(allPayslipData);

    // Assuming `allPayslipData` is an array of payslip data objects
    if (Array.isarray(allPayslipData) && allPayslipData.length === 0) {
        console.log(allPayslipData); 

        allPayslipData.forEach(payslipData => {
            // Assuming `multires` is part of your payslipData or fetched/constructed similarly
            var multires = {
                payslip: payslipData,
                salaryDetails: payslipData.salaryDetails,
                lstTaxDetails: payslipData.taxDetails,
                absencesLeft: payslipData.absences
            }
        };
        } else {
            allPayslipsHtml += generatePayslipHTML(payslipData, multires);
