...
function generatePDFA5(htmlString) {
    // Convert HTML string to a temporary element for measurement
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = htmlString;
    document.body.appendChild(tempDiv); // Temporarily append for measurement
    ...
    html2canvas(tempDiv, { scale: 1 }).then(canvas => {
        ...
        while (heightLeft >= 0) {
            position = heightLeft - imgHeight; // top position of the next page
            pdf.addPage();
            pdf.addImage(imgData, 'JPEG', 0, position, imgWidth, imgHeight);
            heightLeft -= pageHeight;
        }
