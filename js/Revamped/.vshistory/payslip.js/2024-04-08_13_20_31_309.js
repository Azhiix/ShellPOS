...
function printAllPayslips() {
    ...
    var promises = payslipIds.map(function (payslipId) {
        return new Promise(function (resolve, reject) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // Assuming generatePayslipHTML returns an HTML string for the payslip
                    var payslipHtml = generatePayslipHTML(payslipData, multiResult);

                    // Convert the HTML to a Data URI via a PDF library (e.g., jsPDF)
                    const doc = new jsPDF();
                    doc.fromHTML(payslipHtml);
                ...
                error: function (xhr, status, error) {
                    console.error("Error occurred: ", status, error);
                    reject(new Error("Failed to load payslip"));
                }
            });
        });
    });

    // Once all AJAX calls are done, print the payslips
    Promise.all(promises).then(function (dataUris) {
        ...
        console.error("Failed to print all payslips: ", error);
    });
}
