...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        // Convert each HTML content to a canvas and then to an image data URL
        const canvasPromises = allPayslipHtmlContents.map(content => {
            const tempDiv = $('<div>').appendTo('body').css({ display: 'none' }).html(content);
            return html2canvas(tempDiv[0]).then(canvas => {
                tempDiv.remove();
                return canvas.toDataURL('image/png');
            });
        });

        // Wait for all canvases to be converted
