...
function generatePDFA5(htmlString) {
    // Assume htmlString is the entire HTML content for all payslips
    // Split the HTML string by your designated page break
    const payslipsHtml = htmlString.split('<div style="page-break-after: always;"></div>');
    ...
    // Function to process each payslip HTML and add to PDF
    function processPayslip(index) {
        if (index >= payslipsHtml.length) {
            // Once all payslips have been processed, save or display the PDF
            window.open(pdf.output('bloburl'), '_blank'); // Open the PDF in a new window
            return;
        }

        // Convert the current payslip HTML to an image
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = payslipsHtml[index];
        ...
        html2canvas(tempDiv, { scale: 3 }).then(canvas => {
            ...
            if (index > 0) {
                pdf.addPage(); // Add a new page for every payslip after the first
            }

            const contentStartY = 5; // Adjust as needed
            ...
            // Process the next payslip
            processPayslip(index + 1);
        });
    }

    // Start processing with the first payslip
    processPayslip(0);
}
...
function addPayslipsToHTML(payslipHtmlContent) {
    ...
    payslipContainer.empty(); // Clear the container first
    ...
    payslips.forEach(function (payslipHtml, index) {
        ...
        // If it's not the last payslip, add a page break
...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        var finalHtmlContent = allPayslipHtmlContents.join('');
        addPayslipsToHTML(finalHtmlContent);
