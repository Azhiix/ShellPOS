    ...
    $('#idPs2Table').on('click', '.viewPayslip', function () {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                console.log('reponse is, ', multiResult)
...
function generatePDFA5(htmlString) {
    ...
    html2canvas(tempDiv, { scale: 3 }).then(canvas => {
        ...
        const disclaimerY = pdf.internal.pageSize.getHeight() - 7; //
...
function DownloadPDFA5(htmlString, emp_name) {
    ...
    html2canvas(tempDiv, { scale: 5 }).then(canvas => {
        const imgData = canvas.toDataURL('image/png');
        const pdf = new jsPDF({
            orientation: 'portrait',
            unit: 'mm',
...
function generatePayslipHTML(payslipData, multires) {
    ...
    var initialHtml = "<div style='font-size: 12px;'><div  emp_pin='" + payslipData.emp_pin + "'>";
...
function printAllPayslips() {
    ...
    var payslipPromises = $('.payslipTblBody tr').map(function () {
        var payslipIds = $(this).data('payslip-id');
        var jwtToken = getCookie("jwttoken");
        ...
        return fetch("payslips.aspx/loadPayslip", {
            ...
            body: JSON.stringify({ payslipIds: payslipIds })
        })
            .then(response => response.json())
            .then(data => {
                var multiResult = data.d;
                // Return HTML directly, assuming generatePayslipHTML is unchanged
    ...
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        ...
        let totalCount = allPayslipHtmlContents.length;
        ...
        allPayslipHtmlContents.forEach((htmlContent, index) => {
            ...
            html2canvas(tempDiv, { scale: 2.5 }).then(canvas => {
                document.body.removeChild(tempDiv); // Clean up after conversion
                ...
                pdf.addImage(imgData, 'JPEG', 5, 5, pdf.internal.pageSize.getWidth() - 10, ((pdf.internal.pageSize.getWidth() - 10) / canvas.width) * canvas.height);

                // Add footer text for the computer-generated document notice
                const disclaimerText = "This is a computer-generated document and requires no signature.";
                const disclaimerY = pdf.internal.pageSize.getHeight() - 7;
                pdf.setFontSize(10);
                const textWidth = pdf.getStringUnitWidth(disclaimerText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
                ...
                if (processedCount === totalCount) {
                    pdf.save('AllPayslips.pdf'); // Save after all have been processed
...
function findPayslips(payslipIds) {
    //ajax comes here
    var jwtToken = getCookie("jwttoken");
    ...
    jQuery.ajax({
        type: "POST",
        url: "payslips.aspx/loadPayslip", //It calls our web method
        data: JSON.stringify({ payslipIds: payslipIds }),
        headers: {
            'Authorization': 'Bearer ' + jwtToken
        },
        contentType: "application/json; charset=utf-8",
...
function fetchAllPayslips(payslipIds) {
    startLoadingAnimation();
    var jwtToken = getCookie("jwttoken");

   
    var payload = JSON.stringify({ 'payslipIds': payslipIds });

    fetch("payslips.aspx/LoadPayslips", {
        method: "POST",
        headers: {
            'Authorization': 'Bearer ' + jwtToken,
            'Content-Type': "application/json; charset=utf-8"
        ...
        .then(response => response.json())
        .then(data => {
            var allPayslipHtmlContents = data.d.map(multiResult => {
                console.log('response is, ', multiResult);
                return generatePayslipHTML(multiResult.payslip, multiResult);
            });

            var payslipContainer = document.getElementById('payslipContainer');
            payslipContainer.innerHTML = allPayslipHtmlContents.join('');
            stopLoadingAnimation();
        })
        .catch(error => {
            console.error("Error occurred: ", error);
            stopLoadingAnimation();
                ...
                window.addEventListener('load', fetchAllPayslips)