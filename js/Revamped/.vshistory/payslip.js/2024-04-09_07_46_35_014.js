...
function printAllPayslips() {
    ...
    Promise.all(payslipPromises).then(individualPayslips => {

        //var finalHtmlContent = individualPayslips.join('');

        individualPayslips.append('<div class="pagebreak"></div>');
        individualPayslips.append('<div style="page-break-after: always;"></div>');

        //const pdf = new jspdf.jsPDF();

        // Add text
        pdf.text("Hello, world!", 10, 10); // (text, x-coordinate, y-coordinate)
        pdf.text("More text here...", 10, 20);

        // Save the PDF
        pdf.save("document.pdf"); addPayslipsToHTML(finalHtmlContent);
