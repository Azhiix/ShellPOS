    ...
    var payslipPromises = $('.payslipTblBody tr').map(function () {
        ...
        return fetch("payslips.aspx/loadPayslip", {
            ...
            .then(data => {
                ...
                const payslipData = multiResult.payslip;
                // Return the HTML content for this payslip
                return generatePayslipHTML(payslipData, multiResult);
            })
            .catch(error => {
                console.error("Error occurred: ", error);
                // Handle error, potentially return a placeholder or error message to include in the printout
                return `<div>Error loading payslip for ID ${payslipId}</div>`;
            });
    }).get(); // Convert to regular array for Promise.all

    // Wait for all promises to resolve
    Promise.all(payslipPromises).then(allPayslipHtmlContents => {
        // Join all HTML contents into one string
        var finalHtmlContent = allPayslipHtmlContents.join('');

        // Call addPayslipsToHTML once with the aggregated content
