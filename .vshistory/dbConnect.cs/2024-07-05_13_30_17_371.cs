        ...
        try
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand insertPaymentCommand = new SqlCommand(insertPaymentQuery, connection))
                {
                    ...
                    if (!string.IsNullOrEmpty(dateFrom))
                    {
                        if (DateTime.TryParseExact(dateFrom, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime fromDate))
                        {
                    ...
                    if (!string.IsNullOrEmpty(dateTo))
                    {
                        if (DateTime.TryParseExact(dateTo, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime toDate))
                        {
                    ...
                    {
                        if (DateTime.TryParseExact(specificDate, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime specific))
                        {
                        ...
                        using (SqlDataReader dataReader = selectPaymentCommand.ExecuteReader())
                        {
                            if (dataReader.Read())
                            {
                                clsPayment newPayment = new clsPayment
                                {
                                    ...
                                    DateFrom = dataReader["DateFrom"] != DBNull.Value ? Convert.ToDateTime(dataReader["DateFrom"]).ToString("dd/MM/yyyy") : null,
                                    DateTo = dataReader["DateTo"] != DBNull.Value ? Convert.ToDateTime(dataReader["DateTo"]).ToString("dd/MM/yyyy") : null,
