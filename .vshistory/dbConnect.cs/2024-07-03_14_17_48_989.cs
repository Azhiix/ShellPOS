    ...
    public static List<clsClient> DisplayAllClients()
    {
        ...
        try
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                // First query: Clients and Vehicles
                string sqlClients = "SELECT c.ClientID, c.Name, c.ContactInfo, c.Address, c.BRN, " +
                ...
                using (SqlCommand command = new SqlCommand(sqlClients, connection))
                {
                    using (SqlDataReader dataReader = command.ExecuteReader())
                    {
                        while (dataReader.Read())
                ...
                {
                    var userPermission = userPermissions.FirstOrDefault(up => up.UserID == client.ClientID);
                    if (userPermission != null)
                    {
                        client.UserId = userPermission.UserID;
                        client.Username = userPermission.Username;
                    }
                }
            }
        }
