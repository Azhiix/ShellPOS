    ...
    public static List<clsClient> DisplayAllClients()
    {
        ...
        try
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                // Using LEFT JOIN to include clients without vehicles and user permissions
                string sql = "SELECT c.ClientID, c.Name, c.ContactInfo, c.Address, c.BRN, " +
                             ...
                             "LEFT JOIN UserPermissions u ON c.ClientID = u.UserID " + // Adjust this line based on your actual relationship
                             "GROUP BY c.ClientID, c.Name, c.ContactInfo, c.Address, c.BRN, u.UserID, u.Username";
