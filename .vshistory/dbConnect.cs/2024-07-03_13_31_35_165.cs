    ...
    public static List<clsProducts> displayProducts() 
{
    ...
    public static bool DeleteSales(int saleItemId, int saleId)
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            try
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    ...
                    string deleteSaleItemQuery = @"
                DELETE FROM [Shell_POS].[dbo].[SaleItems]
                WHERE SaleItemId = @SaleItemId;";

                    using (SqlCommand deleteSaleItemCommand = new SqlCommand(deleteSaleItemQuery, connection, transaction))
                    {
                        deleteSaleItemCommand.Parameters.AddWithValue("@SaleItemId", saleItemId);
                        deleteSaleItemCommand.ExecuteNonQuery();
                    }

                    // Check if there are any SaleItems left for the given SaleId
                    string checkRemainingSaleItemsQuery = @"
                SELECT COUNT(*) 
                FROM [Shell_POS].[dbo].[SaleItems]
                WHERE SaleId = @SaleId;";

                    int remainingSaleItemsCount;
                    using (SqlCommand checkRemainingSaleItemsCommand = new SqlCommand(checkRemainingSaleItemsQuery, connection, transaction))
                    {
                        checkRemainingSaleItemsCommand.Parameters.AddWithValue("@SaleId", saleId);
                        remainingSaleItemsCount = (int)checkRemainingSaleItemsCommand.ExecuteScalar();
                    }

                    // If no SaleItems are left, delete the Sale
                    if (remainingSaleItemsCount == 0)
                    {
